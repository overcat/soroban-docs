...
[auth example]: https://github.com/stellar/soroban-examples/tree/v0.6.0/auth
...
[`auth/src/test.rs`]: https://github.com/stellar/soroban-examples/tree/v0.6.0/auth/src/test.rs
...
`--user` argument in order to allow `soroban` tool to automatically sign the
...
View the data that has been stored against each user with `soroban contractcontract  read`.
...
[Atomic Swap](atomic-swap.mdx) example demonstrates multi-party authorization
